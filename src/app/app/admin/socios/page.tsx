...
export default async function SociosPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-tight">Socios</h1>
        <p className="text-sm text-muted-foreground">
          El alta se hace sobre un usuario que ya existe (se registró solo). Ser
          socio es una condición que la administración habilita, no un rol que
          se elige al registrarse.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {(socios ?? []).map((s) => {
          ...
          return (
            <form
              key={s.id}
              action={actualizarSocio.bind(null, s.id)}
        ...
        {socios?.length === 0 && (
          <p className="text-sm text-muted-foreground">Todavía no hay socios cargados.</p>
        )}
      </div>
    </div>
  );
